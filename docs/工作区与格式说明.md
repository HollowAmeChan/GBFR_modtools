...
Granite DDS 是可编辑中间态，编辑器不会修改 GTS/GTP。对于 `source/data/texture` 中没有同名原件的 DDS，工作区会记录“新建贴图”候选，可构建为新的普通 `.texture`。已有 WTB（包括 `UI-image`）使用源文件模板封回；mmat JSON 可在专用编辑页修改并编码，删除 `A4` 后可让材质转向 `A2.Name` 指定的普通贴图。
