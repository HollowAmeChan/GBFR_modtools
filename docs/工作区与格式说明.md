...
使用编辑器快捷操作删除眼球材质条目的 `A4`、改用 `data/texture` 下的普通 `.texture` 时，需要同时构建该材质 `A2` 实际引用的 `conj/iris/eyeh/msk1` 文件；只编码 `iris` 而把其余层留在 Granite 中，不能保证清除 A4 后仍能得到完整眼球效果。默认配色只处理 `vars/0.mmat.json`；其他配色仍需在对应编号的 mmat 中分别处理。
...
已有 WTB（包括 `UI-image`）使用源文件模板原样封回，不执行 V 向翻转，否则会把原本正确的槽二次翻转。mmat JSON 可在专用编辑页修改并编码，删除 `A4` 后可让材质转向 `A2.Name` 指定的普通贴图。
...
工作区大列表使用 Dear ImGui 原生多选：普通点击切换单项，`Ctrl` 增减选择，`Shift` 选择连续范围，也支持框选与 `Ctrl+A`。Inspector 和“快捷操作”会在多选时切换为批处理，可批量构建、恢复及清除所选 mmat 的 A4；“构建全部修改项”会构建所有可用且已修改的 C++ 可构建资源，并跳过仍由 Blender 负责的 cloth 中间态。
