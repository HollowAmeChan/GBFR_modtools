...
Blender 插件的“导出到工作区”会让用户选择 `workspace.json`，在确认前列出当前模型对应的三条 `unpack` 目标，然后自动覆盖 `.minfo/.skeleton/.mmesh`。不再需要管理 `_Exported_MInfo` 或手动复制文件。插件生成的调试 JSON 保存到 `.gbfr/exports/`，不会进入 Mod；`build` 仍只接收在编辑器中确认后的最终文件。
...
Granite DDS 是可编辑中间态，编辑器不会修改 GTS/GTP。对于 `source/data/texture` 中没有同名原件的 DDS，工作区仍会记录“新建贴图”候选；新建 WTB 与 mmat JSON 编码仍未实现。已有 WTB（包括 `UI-image`）可以使用源文件模板封回，删除 `A4` 转向 `A2.Name` 普通贴图的完整 mmat 流程仍需后续实现。

## 3. 构建 Mod

模型文件 `.minfo/.skeleton/LOD0 .mmesh` 和已有 WTB/UI-image 可由 C++ 原生构建：在工作区列表选中项目，然后在 Inspector 中使用模型的“从 unpack 复制到 build”，或 WTB 的“封回 WTB 到 build”。恢复按钮分别为“从 source 恢复 unpack”和“从 source 恢复 DDS”。写入后保留游戏原始相对路径：
...
WTB `.texture`、UI WTB、mmat JSON 和 cloth XML 仍会进入工作区，供预览、研究和编辑中间态；已有 WTB 的槽封回已经接入，mmat JSON 和 cloth XML 编码仍未实现。旧 PowerShell 构建器及兼容入口已删除。
...
仓库目录职责见 [C++ 架构](架构.md#目录)。当前工作区生成可自动解码普通 WTB 和角色 UI WTB 为 DDS、mmat 为 JSON、cloth BXM 为 XML，并从 Granite GTS/GTP 提取 `albd/msk1/msk2/nrml/conj/iris/eyeh`。已有 WTB 可以按源模板反向封回；mmat 和 cloth 仍是预览和编辑中间态。

当前可构建的最终资源包括 `.minfo/.skeleton/LOD0 .mmesh` 和已有 WTB/UI-image。mmat 和 cloth BXM 编码是后续 C++ 模块的待实现项，不再通过隐藏的旧脚本入口提供。
