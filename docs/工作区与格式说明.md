...
`source/` 是不可编辑的原始模板。删除 `unpack/` 中间文件后，该项会标为“缺少输入”。当前 C++ 版本已支持模型 `.minfo/.skeleton` 和全部已登记 `.mmesh` 的逐项恢复；模型预览暂时固定读取 LOD0，其他中间态的原生恢复命令尚未迁移。
...
当前可构建的最终资源包括 `.minfo/.skeleton`、全部工作区 LOD `.mmesh`、普通 WTB/UI-image、mmat，以及 Blender 插件写回的 cloth BXM。C++ 预览器目前只显示 LOD0，但恢复和构建不会丢弃其他 LOD。
