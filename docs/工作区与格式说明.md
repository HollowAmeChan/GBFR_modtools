...
Blender 插件导出完成后，将 `_Exported_MInfo` 中的 `.minfo/.skeleton/.mmesh` 复制到工作区对应 `unpack/data/` 路径，然后在编辑器中刷新。插件生成的调试 `.json` 不进入 Mod。`.mmesh` 的正式位置是 `data/model_streaming/lod0/`，不是 `.minfo` 同目录的 Blender 导入辅助副本。
...
当前只有模型文件 `.minfo/.skeleton/LOD0 .mmesh` 可由 C++ 原生构建：在工作区列表选中项目，然后在 Inspector 中使用“写入 build”或“恢复 unpack”。写入后保留游戏原始相对路径：
