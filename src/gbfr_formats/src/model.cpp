...
MeshAsset load_mmesh(const fs::path& path,const ModelInfoAsset& info,std::size_t lod_index,bool shadow_lod) {
    ...
    const auto optional_vertex_count=[&](const MeshBufferLocator& b,std::size_t stride,const char* label) {
        if(!stride||b.size%stride||b.size/static_cast<std::uint64_t>(stride)>lod.vertex_count)
            throw std::runtime_error(std::string("Invalid partial ")+label+" buffer size");
        return static_cast<std::uint32_t>(b.size/stride);
    };
    const auto read_joints=[&](std::size_t first){const bool primary=first==0;const auto& b=buffer(buffer_index++,primary?static_cast<std::uint64_t>(lod.vertex_count)*8:0);const auto count=primary?lod.vertex_count:optional_vertex_count(b,8,"joint");for(std::uint32_t i=0;i<count;++i)for(std::size_t j=0;j<4;++j){const auto mapped=view.read<std::uint16_t>(static_cast<std::size_t>(b.offset)+i*8+j*2,"joint index");if(mapped>=info.bones_to_weight_indices.size())throw std::runtime_error("Joint index outside minfo bone map (value="+std::to_string(mapped)+", map_size="+std::to_string(info.bones_to_weight_indices.size())+")");result.vertices[i].joints[first+j]=info.bones_to_weight_indices[mapped];}};
    if(lod.buffer_types&2)read_joints(0);
    if(lod.buffer_types&4)read_joints(4);
    const auto read_weights=[&](std::size_t first){const bool primary=first==0;const auto& b=buffer(buffer_index++,primary?static_cast<std::uint64_t>(lod.vertex_count)*8:0);const auto count=primary?lod.vertex_count:optional_vertex_count(b,8,"weight");for(std::uint32_t i=0;i<count;++i)for(std::size_t j=0;j<4;++j)result.vertices[i].weights[first+j]=view.read<std::uint16_t>(static_cast<std::size_t>(b.offset)+i*8+j*2,"joint weight")/65535.0f;};
    ...
    if(lod.buffer_types&32){const auto& b=buffer(buffer_index++,0);const auto count=optional_vertex_count(b,4,"color");for(std::uint32_t i=0;i<count;++i){const auto p=static_cast<std::size_t>(b.offset)+i*4;result.vertices[i].color={view.read<std::uint8_t>(p,"color r")/255.0f,view.read<std::uint8_t>(p+1,"color g")/255.0f,view.read<std::uint8_t>(p+2,"color b")/255.0f,view.read<std::uint8_t>(p+3,"color a")/255.0f};}result.has_color=true;}
    if(lod.buffer_types&64){const auto& b=buffer(buffer_index++,0);const auto count=optional_vertex_count(b,4,"UV1");for(std::uint32_t i=0;i<count;++i){const auto p=static_cast<std::size_t>(b.offset)+i*4;result.vertices[i].uv1={half_to_float(view.read<std::uint16_t>(p,"uv1 x")),half_to_float(view.read<std::uint16_t>(p+2,"uv1 y"))};}result.has_uv1=true;}
