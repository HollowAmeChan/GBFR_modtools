...
ModelInfoAsset load_minfo(const fs::path& path) {
    ...
    const auto lods=view.vector(view.field(root,1,true)); if (!view.vector_size(lods)) throw std::runtime_error("minfo has no LOD0");
    const auto lod=view.table_in_vector(lods,0);
    if (const auto value=view.field(lod,2)) result.vertex_count=view.read<std::uint32_t>(value,"vertex count");
    if (const auto value=view.field(lod,3)) result.index_count=view.read<std::uint32_t>(value,"index count");
    if (const auto value=view.field(lod,4)) result.buffer_types=view.read<std::uint8_t>(value,"buffer types");
    const auto buffers=view.vector(view.field(lod,0,true));
    for (std::uint32_t i=0;i<view.vector_size(buffers);++i) { const auto p=buffers+4+i*16; result.buffers.push_back({view.read<std::uint64_t>(p,"buffer offset"),view.read<std::uint64_t>(p+8,"buffer size")}); }
    if (const auto field=view.field(lod,1)) { const auto vector=view.vector(field); for (std::uint32_t i=0;i<view.vector_size(vector);++i) { const auto p=vector+4+i*12; result.chunks.push_back({view.read<std::uint32_t>(p,"chunk offset"),view.read<std::uint32_t>(p+4,"chunk count"),view.read<std::uint8_t>(p+8,"submesh id"),view.read<std::uint8_t>(p+9,"material id")}); } }
    ...
    if (!result.vertex_count || result.index_count%3 || result.buffers.empty()) throw std::runtime_error("Invalid LOD0 counts or buffers");
    return result;
}

MeshAsset load_mmesh(const fs::path& path,const ModelInfoAsset& info) {
    BinaryView view(path);
    auto buffer=[&](std::size_t i,std::uint64_t bytes)->const MeshBufferLocator& { if(i>=info.buffers.size()) throw std::runtime_error("Missing mmesh buffer locator"); const auto& b=info.buffers[i]; if(b.size<bytes||b.offset>view.size()||bytes>view.size()-static_cast<std::size_t>(b.offset)) throw std::runtime_error("mmesh buffer outside file"); return b; };
    MeshAsset result; result.vertices.resize(info.vertex_count); const auto& main=buffer(0,static_cast<std::uint64_t>(info.vertex_count)*32);
    for(std::uint32_t i=0;i<info.vertex_count;++i) { const auto p=static_cast<std::size_t>(main.offset)+i*32; auto& v=result.vertices[i]; v.position={view.read<float>(p,"vertex x"),view.read<float>(p+4,"vertex y"),view.read<float>(p+8,"vertex z")}; v.normal={half_to_float(view.read<std::uint16_t>(p+12,"normal x")),half_to_float(view.read<std::uint16_t>(p+14,"normal y")),half_to_float(view.read<std::uint16_t>(p+16,"normal z"))}; v.tangent={half_to_float(view.read<std::uint16_t>(p+20,"tangent x")),half_to_float(view.read<std::uint16_t>(p+24,"tangent z")),half_to_float(view.read<std::uint16_t>(p+22,"tangent y"))}; v.uv={half_to_float(view.read<std::uint16_t>(p+28,"uv x")),half_to_float(view.read<std::uint16_t>(p+30,"uv y"))}; }
    if(info.buffer_types&2) { const auto& b=buffer(1,static_cast<std::uint64_t>(info.vertex_count)*8); for(std::uint32_t i=0;i<info.vertex_count;++i) for(std::size_t j=0;j<4;++j) { const auto mapped=view.read<std::uint16_t>(static_cast<std::size_t>(b.offset)+i*8+j*2,"joint index"); if(mapped>=info.bones_to_weight_indices.size()) throw std::runtime_error("Joint index outside minfo bone map"); result.vertices[i].joints[j]=info.bones_to_weight_indices[mapped]; } }
    if(info.buffer_types&8) { const auto& b=buffer((info.buffer_types&4)?3:2,static_cast<std::uint64_t>(info.vertex_count)*8); for(std::uint32_t i=0;i<info.vertex_count;++i) for(std::size_t j=0;j<4;++j) result.vertices[i].weights[j]=view.read<std::uint16_t>(static_cast<std::size_t>(b.offset)+i*8+j*2,"joint weight")/65535.0f; }
    const auto& ib=buffer(info.buffers.size()-1,static_cast<std::uint64_t>(info.index_count)*4); result.indices.resize(info.index_count);
    for(std::uint32_t i=0;i<info.index_count;i+=3) { const auto p=static_cast<std::size_t>(ib.offset)+i*4; result.indices[i]=view.read<std::uint32_t>(p+8,"triangle c"); result.indices[i+1]=view.read<std::uint32_t>(p+4,"triangle b"); result.indices[i+2]=view.read<std::uint32_t>(p,"triangle a"); if(result.indices[i]>=info.vertex_count||result.indices[i+1]>=info.vertex_count||result.indices[i+2]>=info.vertex_count) throw std::runtime_error("Triangle index outside vertex buffer"); }
    result.chunks=info.chunks; return result;
}
}
