...
SOP 页当前只开放已确认的 Swing/Twist 复制旋转约束：选择目标裙骨、来源腿骨、主轴及 Swing/Twist 比例后保存。奶刀裙骨案例常用 Y 轴、Swing `0.5` 或 `0.6`、Twist `0`；只有确实需要完整跟随时才使用 `1/1`。已有未知操作和额外 corrective 属性不会被改写；`.sop` 作为模型资源可从 `unpack` 直接构建到 `build`，也可从 `source` 恢复。
