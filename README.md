...
- 每一行的“恢复”按钮会把对应 `unpack` 文件恢复为探索时内容：普通 DDS/JSON 来自 `source`，Granite DDS 按原 GTS/hash 重新提取；已删除文件会以“缺少输入”保留在列表中，也可以直接恢复。
- mmat JSON 行提供“清除 A4 (N)”按钮；确认后删除该 JSON 中所有材质条目的 A4、自动标记为已修改并勾选，其他字段保持不变。
- 在写入前显示完整目标清单。
