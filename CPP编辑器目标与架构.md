...
> 实施状态（2026-07-16）：M0-M5 已完成首轮实现。C++ 已覆盖工作区、模型解析/构建、D3D11 预览、骨架与 CLH/CLP 联动，并新增 `.mot` 独立解码器、骨骼姿态采样、CPU 蒙皮和动画切片时间轴；texture、mmat、BXM 编码和新建 texture 继续走旧版兼容入口。具体边界与验证见 [CPP编辑器构建与迁移.md](CPP编辑器构建与迁移.md)。
...
4. `msk1/msk2` 第一阶段不解释具体通道，只允许调试面板单独查看通道。
